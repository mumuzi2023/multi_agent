...
def get_joint_state(observations_dict, ordered_agent_ids, single_agent_n_features,
    ...
    for agent_id in ordered_agent_ids:
        if agent_id in observations_dict:
            ...
            processed_obs = change_observation(raw_obs)
        else:
...
def run_pursuit_parallel():
    ...
    with open(LOG_FILE_n, 'w+', newline='') as myfile:
        writer = csv.writer(myfile)
        writer.writerow(["Episode", "AverageReward_CQL"])
        ...
        if (episode + 1) % SAVE_MODEL_FREQUENCY == 0:
            ...
            for SEED_test in [0, 1, 2, 3, 4]:
                ...
                for step_in_episode in range(MAX_STEPS_PER_EPISODE):
                    ...
                    for agent_id in env.agents:
                        action_mask = np.ones(N_ACTIONS, dtype=np.int8)
                        actions_to_take_dict[agent_id] = RL.choose_action(current_joint_state, action_mask,execution=True)
            ...
            print(
                f"Episode: {episode + 1}/{NUM_EPISODES}, Avg Reward: {avg_episode_reward:.2f}, Avg Reward evaluate: {avg_reward:.2f}, Epsilon: {RL.epsilon:.2f}, Global Steps: {global_step_counter}, Buffer: {RL.memory_counter}")
            if RL.cost_his:
