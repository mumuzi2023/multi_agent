...
def run_pursuit_parallel():
    ...
    for episode in range(NUM_EPISODES):
        ...
        if (episode + 1) % 10 == 0:
            ...
            for SEED_test in [0,1,2,3]:
                ...
                for step_in_episode in range(MAX_STEPS_PER_EPISODE):
                    ...
                    for agent_id in env.agents:
                        processed_obs_for_agent = current_processed_observations[agent_id]
                        action_mask = np.ones(N_ACTIONS, dtype=np.int8)
                        actions_to_take_dict[agent_id] = RL.choose_action(processed_obs_for_agent, action_mask,execution=True)
            ...
            print(
                f"Episode: {episode + 1}/{NUM_EPISODES}, Avg Reward: {avg_episode_reward:.2f}, Avg Reward1: {avg_reward:.2f}, Epsilon: {RL.epsilon:.2f}, Global Steps: {global_step_counter}, Buffer: {RL.memory_counter}")
            if RL.cost_his:
