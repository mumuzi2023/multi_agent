...
df = pd.read_csv('pettingzoosislpursuitDQN_parallel_torch.csv')
...
df['AverageReward_DQN_Rolling_10'] = df['AverageReward_DQN'].rolling(window=10, min_periods=1).mean()

# Create the plot
plt.figure(figsize=(12, 6))

# Plot the original average reward
plt.plot(df['Episode'], df['AverageReward_DQN'], label='每集平均奖励 (原始)', linestyle='-')
