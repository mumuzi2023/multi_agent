...
df = pd.read_csv('cooperative_harvesting_dqn_parallel_torch.csv')
...
df['AverageReward_DQN_Rolling_10'] = df['AverageReward_AllAgents'].rolling(window=10, min_periods=1).mean()

# Create the plot
plt.figure(figsize=(12, 6))

# Plot the original average reward
plt.plot(df['Episode'], df['AverageReward_AllAgents'], label='每集平均奖励 (原始)', linestyle='-')

# Plot the 10-point rolling average
plt.plot(df['Episode'], df['AverageReward_DQN_Rolling_10'], label='10集滚动平均奖励', linestyle='--')
